...
bool should_propagate(int i, int j, int label, Eigen::MatrixXd &N, Eigen::MatrixXd &label_matrix)
{
	//double dist = 
...
//{	
//	proc(cur_face_idx);
//	suGlobalState::gOnly().label_matrix(cur_face_idx, 0) = label;
//	for (int i=0; i<3; i++)
//	{
...
void propagate_from_neighbor_loop(int cur_face_idx, int label, const ProcessCallback &proc = ProcessCallback())
{
	...
	while (!face_list.empty())
	{
		...
		for (int i = 0; i<3; i++)
		{
			int j = TT(cur_id, i);
			if (j == -1) continue;  //debug
			if (should_propagate(cur_id, j, label, N, label_matrix))
			{
					...
					if (key == 'E')
					{
						if (b_select_mode)
						{
							...
							if (gFid != -1)
							{
								...
								int cur_object_label = suGlobalState::gOnly().cur_index_of_object_label;
							...
							// hover effect
							//C = Eigen::MatrixXd::Constant(viewer->data().F.rows(), 3, 1);
							//int r = suGlobalState::gOnly().predfined_colors.row(suGlobalState::gOnly().cur_index_of_object_label)
							for (auto i : Sel_Set)
							{
